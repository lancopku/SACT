...
class rnn_decoder(nn.Module):
    ...
    def __init__(self, config, embedding=None, use_attention=True):
        ...
        self.embedding = embedding if embedding is not None else nn.Embedding(
            config.tgt_vocab_size, config.emb_size)
    ...
    def forward(self, input, state, c_t):
        embs = self.embedding(input)
        output, state = self.rnn(embs, state)

        if self.attention is not None:
            output, attn_weights, c_t = self.attention(output, c_t)
        else:
            attn_weights = None

        output = self.compute_score(output)

        return output, state, attn_weights, c_t
